...
def isWin(bingo):
    for b in range(len(bingo)):
        ...
        for i in range(len(bingo[b])):
            verti = []
            for j in range(len(bingo[b][0])):
                verti.append(bingo[b][j][i])
            if(sum(verti)==-5):
                return b
    return -1
        ...
        for i in range(len(bingo)):
            for j in range(len(bingo[0])):
                for k in range(len(bingo[0][0])):
                    if bingo[i][j][k] == c:
                        bingo[i][j][k] = -1
        check_win = isWin(bingo)
        if check_win != -1:
            win_bingo = bingo[check_win]
            res = 0
            for i in range(len(win_bingo)):
                for j in range(len(win_bingo[0])):
                    if win_bingo[i][j] != -1:
                        res += win_bingo[i][j]
            print(res*c)
            return
